...
def main() -> None:
    ...
    for name, block in learner_blocks(LEARNERS).items():
        ...
        row = {
            "clean": _block_metric(clean_env, agents, block, metric_key, clean_episodes)
        }
        ...
        results[f"{name} ({metric_key},{direction})"] = (row, direction)

    _print_table(results, categories)


def _print_table(
    results: dict[str, tuple[dict[str, float], str]], categories: list[str]
) -> None:
    ...
    for name, (row, _) in results.items():
    ...
    for name, (row, direction) in results.items():
